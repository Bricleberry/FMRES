...
class GCN(MyModel):
    ...
    def build(self):
        ...
        user_rep_t, user_outputs = tf.compat.v1.nn.dynamic_rnn(
            GRUCell(self.eb_dim), inputs=user_output, dtype=tf.float32,
            scope='u')
        item_rep_t, item_outputs = tf.compat.v1.nn.dynamic_rnn(
            GRUCell(self.eb_dim), inputs=item_output, dtype=tf.float32,
            scope='i')

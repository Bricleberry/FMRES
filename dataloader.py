    ...
    def gen_item_seq(self,iids):
        ...
        for iid in iids:
            ...
            for iid_ in iid_seq:
                s = list(history)
                len_state.append(
                    self.max_time if len(s) >= self.max_time else 1 if len(s) == 0 else len(
                        s))

                s = u.pad_history(s, self.max_time, 0)
                a = iid_
                is_b = 1
                state.append(s)
                action.append(a)
                ...
                history.append(iid_)
                next_s = list(history)
                len_next_state.append(
                    self.max_time if len(next_s) >= self.max_time else 1 if len(
                        next_s) == 0 else len(next_s))
                next_s = u.pad_history(next_s, self.max_time, 0)

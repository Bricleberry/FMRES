...
from layers import AttentionAggregator,CustomGRUWithTimeRetention





class FMRESBASE(object):
    def __init__(self, feature_size, eb_dim, max_time_len, user_fnum, item_fnum, max_len, time_inter, k_hop,name="pp"):
        # reset graph
        ...
        with tf.compat.v1.name_scope('{}_inputs'.format(name)):
            ...
            self.feed_dict['sample_num'] = tf.compat.v1.placeholder(tf.int32, [])
        ...
        self.item_memory_keeper_gru = CustomGRUWithTimeRetention(eb_dim, time=
        self.feed_dict['iid_seqs_len'], inter=self.time_inter)
    ...
    def build_fc_net(self, inp):
        ...
        fc3 = tf.compat.v1.layers.dense(dp2, 1, activation=None, name='ce-logits')
    ...
    def multi_GCN(self):
        ...
        for i in range(self.max_time_len):
            with tf.compat.v1.variable_scope('gcn_{}'.format(i)):
                ...
                lookup_uid = tf.nn.embedding_lookup(tf.transpose(self.feed_dict['uid_seqs']), [i])#[T,BxS] x [t] -> [BxS]
                lookup_iid = tf.nn.embedding_lookup(tf.transpose(self.feed_dict['iid_seqs']), [i])
                uid_ = tf.nn.embedding_lookup(output, lookup_uid)  #[BxS,H]
                ...
                # uid_embedding_seqs.append(uid_)
                iid_embedding_seqs.append(tf.nn.embedding_lookup(output, lookup_iid))#[BxS,H]

                uid_time_embeddings_seqs.append(
                    tf.nn.embedding_lookup(self.feed_dict['time_arr_{}'.format(i)], lookup_uid))#[BxS] （t）
                iid_time_embeddings_seqs.append(
                    tf.nn.embedding_lookup(self.feed_dict['time_arr_{}'.format(i)], lookup_iid))



        uid = tf.concat(uid_embedding_seqs, 0)#[T,BxS,H]
        iid = tf.concat(iid_embedding_seqs, 0)#[T,BxS,H]
        uid_time = tf.concat(uid_time_embeddings_seqs, 0)
        iid_time = tf.concat(iid_time_embeddings_seqs, 0)

        uid_seqs = tf.reshape(tf.transpose(uid, [1, 0, 2]), [-1, output.get_shape().as_list()[1]])#[BxSxT,H]
        ...
        uid_seqs = tf.reshape(tf.nn.embedding_lookup(uid_seqs, tf.reshape(self.feed_dict['uid_which_slices'], [-1])),
                              [-1, self.max_time_len, output.get_shape().as_list()[1]])#[BxS,T,H]
...
class PP(FMRESBASE):
    def __init__(self, feature_size, eb_dim, max_time_len, user_fnum, item_fnum, max_len, time_inter, k_hop,name):
        super(PP, self).__init__(feature_size, eb_dim, max_time_len, user_fnum, item_fnum, max_len, time_inter, k_hop,name)

        user_slice_seqs, item_slice_seqs, uid_time_seqs, iid_time_seqs = self.multi_GCN()

        item_slice_seqs = self.item_memory_keeper_gru(item_slice_seqs)

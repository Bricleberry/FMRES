...
## Foursquare
DATA_DIR_Foursquare = '../../FMRES-data/Foursquare/feateng/'
GRAPH_DIR_Foursquare = '../../FMRES-data/Foursquare/feateng/graph/'
USER_PER_COLLECTION_Foursquare = 500
ITEM_PER_COLLECTION_Foursquare = 500
START_TIME_Foursquare = 0
OBJ_PER_TIME_SLICE_Foursquare = 10
USER_NUM_Foursquare = 47411
ITEM_NUM_Foursquare = 42136
START_TIME_IDX_Foursquare = 0
TIME_SLICE_NUM_Foursquare = 16

FEAT_SIZE_Foursquare = 1 + USER_NUM_Foursquare + ITEM_NUM_Foursquare
TIME_DELTA_Foursquare = 7 * 24 * 3600


## Douban
DATA_DIR_Douban = '../../FMRES-data/Douban/feateng/'
GRAPH_DIR_Douban = '../../FMRES-data/Douban/feateng/graph/'
USER_PER_COLLECTION_Douban = 500
ITEM_PER_COLLECTION_Douban = 500
START_TIME_Douban = 0
OBJ_PER_TIME_SLICE_Douban = 10
USER_NUM_Douban= 36694
ITEM_NUM_Douban = 27307
START_TIME_IDX_Douban = 0
TIME_SLICE_NUM_Douban = 12

FEAT_SIZE_Douban = 1 + USER_NUM_Douban + ITEM_NUM_Douban
TIME_DELTA_Douban = 28 * 24 * 3600

## Yelp
DATA_DIR_Yelp = '../../FMRES-data/Yelp/feateng/'
GRAPH_DIR_Yelp = '../../FMRES-data/Yelp/feateng/graph/'
USER_PER_COLLECTION_Yelp = 500
ITEM_PER_COLLECTION_Yelp = 500
START_TIME_Yelp = 0

OBJ_PER_TIME_SLICE_Yelp = 10
USER_NUM_Yelp = 68099
ITEM_NUM_Yelp = 30639
START_TIME_IDX_Yelp = 0
TIME_SLICE_NUM_Yelp = 14

FEAT_SIZE_Yelp = 1 + USER_NUM_Yelp + ITEM_NUM_Yelp
TIME_DELTA_Yelp = 21 * 24 * 3600
...
def restore(model_type, target_file_test, pred_time_test,
    ...
    print('restore begin')
    if 'PP' in model_type:
        model = PP(feature_size, eb_dim, max_time_len, user_fnum, item_fnum, MAX_LEN, time_inter, k_hop,name="pp1")
    else:
...
def eval(model, sess, data_loader, pred_time, neg_sample_num, reg_lambda, isTest):
    ...
    for batch in tqdm(range(data_loader.num_of_batch)):
        uids, iids, uid_seqs, iid_seqs, uid_which_slices, iid_which_slices, uid_seqs_len, iid_seqs_len, label, inter_time,uid_neighbors_seq,uid_neighbors_seq_mask,uid_replay_buffer,iid_replay_buffer = data_loader.gen_seqs(
            batch)
        feed_dict = u.construct_dict(model, adj_list, user_ids_list, item_ids_list, None, reg_lambda, uids, iids,
                                     ...
                                     time_list, inter_time,uid_neighbors_seq,uid_neighbors_seq_mask,1+TEST_NEG_SAMPLE_NUM)
        pred = model.eval(sess, feed_dict)
...
def train(model_type, feature_size, eb_dim, max_time_len,
    ...
    if 'PP' in model_type:
        model = PP(feature_size, eb_dim, max_time_len, user_fnum, item_fnum, MAX_LEN, time_inter, k_hop,name="pp1")
        # model2 = PP(feature_size, eb_dim, max_time_len, user_fnum, item_fnum, MAX_LEN, time_inter, k_hop,name="pp2")
    else:
        print('WRONG MODEL TYPE')
        exit(1)
...
if __name__ == '__main__':
    ...
    if data_set == 'Foursquare':
        user_feat_dict_file = None
        item_feat_dict_file = None
        user_fnum = 1
        item_fnum = 1

        target_file_train = DATA_DIR_Foursquare + 'target_14.txt'
        target_file_validation = DATA_DIR_Foursquare + 'target_15_sample.txt'
        target_file_test = DATA_DIR_Foursquare + 'target_16_sample.txt'

        start_time = START_TIME_Foursquare
        pred_time_train = 14
        pred_time_validation = 15
        pred_time_test = 16

        # model parameter
        feature_size = FEAT_SIZE_Foursquare
        max_time_len = TIME_SLICE_NUM_Foursquare - START_TIME_Foursquare - 1
        graph_dir = GRAPH_DIR_Foursquare
        user_num = USER_NUM_Foursquare
        time_inter = TIME_DELTA_Foursquare
        ...
        target_file_train = DATA_DIR_Douban + 'target_10.txt'
        target_file_validation = DATA_DIR_Douban + 'target_11_sample.txt'
        target_file_test = DATA_DIR_Douban + 'target_12_sample.txt'

        start_time = START_TIME_Douban
        pred_time_train = 10
        pred_time_validation = 11
        pred_time_test = 12

        # model parameter
        feature_size = FEAT_SIZE_Douban
        max_time_len = TIME_SLICE_NUM_Douban - START_TIME_Douban - 1
        graph_dir = GRAPH_DIR_Douban
        user_num = USER_NUM_Douban
        time_inter = TIME_DELTA_Douban
    elif data_set == 'Yelp':
        user_feat_dict_file = None
        item_feat_dict_file = None
        user_fnum = 1
        ...
        target_file_train = DATA_DIR_Yelp + 'target_12.txt'
        target_file_validation = DATA_DIR_Yelp + 'target_13_sample.txt'
        target_file_test = DATA_DIR_Yelp + 'target_14_sample.txt'

        start_time = START_TIME_Yelp
        pred_time_train = 12
        pred_time_validation = 13
        pred_time_test = 14

        # model parameter
        feature_size = FEAT_SIZE_Yelp
        max_time_len = TIME_SLICE_NUM_Yelp - START_TIME_Yelp - 1
        graph_dir = GRAPH_DIR_Yelp
        user_num = USER_NUM_Yelp
